...
void main() {
  group('DicomWebClient & SeriesBuffer Integration', () {
    test('Queries studies and parses study list', () async {
      ...
      final client = DicomWebClient(
          baseUrl: 'http://localhost:8000', httpClient: mockClient);
    ...
    test(
        'fetchFrameBytes requests WADO frames with image/jp2 MIME type for JPEG2000 Lossless',
        () async {
      ...
      final mockClient = MockClient((request) async {
        if (request.url.path.contains('/frames/1')) {
          ...
          return http.Response.bytes(
            Uint8List.fromList(
                [0xFF, 0x4F, 0xFF, 0x51]), // Dummy J2K codestream header
            200,
    ...
    test('DicomSeriesBuffer enforces LRU cache eviction within maxCacheSize',
        () async {
      final series = DicomSeries.fromJson({
        '0020000D': {
          'vr': 'UI',
